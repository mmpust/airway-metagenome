...
dataset <- read_delim(
  "data_input/raspir_alignment_pangenome/merged_project3_rare_2.csv", ";", 
...
# import metadata
metadata <- read_delim(
  "data_input/raspir_alignment_pangenome/spatial_metaData_2020_12.csv",  ";", 
  escape_double = FALSE, trim_ws = TRUE, skip_empty_rows = TRUE)
...
# raspir tables
healthy_0_raspir <- read_csv("data_input/raspir_alignment_pangenome/healthy_0_raspir.csv", col_names = FALSE)
healthy_0_raspir <- healthy_0_raspir$X1

healthy_A_raspir <- read_csv("data_input/raspir_alignment_pangenome/healthy_1_3_raspir.csv", col_names = FALSE)
healthy_A_raspir <- healthy_A_raspir$X1

healthy_C_raspir <- read_csv("data_input/raspir_alignment_pangenome/healthy_4_6_raspir.csv", col_names = FALSE)
healthy_C_raspir <- healthy_C_raspir$X1

cf_0_raspir <- read_csv("data_input/raspir_alignment_pangenome/cf_0_raspir.csv", col_names = FALSE)
cf_0_raspir <- cf_0_raspir$X1

cf_A_raspir <- read_csv("data_input/raspir_alignment_pangenome/cf_1_3_raspir.csv", col_names = FALSE)
cf_A_raspir <- cf_A_raspir$X1

cf_C_raspir <- read_csv("data_input/raspir_alignment_pangenome/cf_4_6_raspir.csv", col_names = FALSE)
...
bp <- ggplot(plot_pie, aes(x="", y=value1, fill=fct_inorder(group1))) + geom_bar(width = 1, stat = "identity") +
  coord_polar("y") + theme(axis.text=element_blank(), axis.title=element_blank(), panel.grid=element_blank(),
                           panel.background=element_blank(), legend.direction = "vertical", legend.position = "bottom",
...
# Make figures ####
#ggsave("image_output/Figure_01.tif", vennDiagramsPlot, device="tiff",
 #      width=8.93, height=8.02, dpi=600)

#ggsave("image_output/Figure_02.tif", randomForest_plot_final, dpi=600,
 #      width=12.1, height=5.73, device="tiff")

#ggsave("image_output/Figure_05.tif", first_network_plot,
 #      device="tiff", width=11, height=6.9, dpi=600)

#ggsave("image_output/Figure_06.tif", modulated_network_similarities, 
 #      device="tiff", dpi = 600, width = 8.59, height=7.98)

#ggsave("image_output/Figure_07.tif", hboth_frechet_plot, dpi=600,
 #      device="tiff", width=11.6, height=7.81)

# Supplementary figures
#ggsave("image_output/Supplementary_Figure_01.tif", plot_pheatmap, device="tiff",
 #      width=8.75, height=9.29, dpi=300)

#ggsave("image_output/Supplementary_Figure_02.tif", where_species, dpi=300,
 #      device="tiff", width=9.79, height=6.9)

#ggsave("image_output/Supplementary_Figure_03.tif", from_where_to_where, device="tiff",
 #      width=8.93, height=8.02, dpi=300)

#ggsave("image_output/Supplementary_Figure_04.tif", pca_plot, dpi=300,
 #      width=9.58, height=7.98, device="tiff")

#ggsave("image_output/Supplementary_Figure_07.tif", stats_rare_core, dpi=300,
 #      device="tiff", width=10.4, height=6.9)

#ggsave("image_output/Supplementary_Figure_08.tif", hboth_shannon_gini_plot, dpi=300,
